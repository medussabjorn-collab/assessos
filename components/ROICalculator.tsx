      ...
      <div className="roi">
        <Stack spacing={3} className="roi-inputs">
          ...
          <div className="roi-field">
            ...
            <Slider
              value={hoursPerHire}
              onChange={(_, v) => setHoursPerHire(v as number)}
              min={2}
              max={40}
              step={1}
              aria-label="Hours per hire"
            />
          </div>
          <div className="roi-field">
            ...
            <Slider
              value={hourlyCost}
              onChange={(_, v) => setHourlyCost(v as number)}
              min={25}
              max={250}
              step={5}
          ...
          <div className="roi-field">
            ...
            <Slider
              value={gainPct}
              onChange={(_, v) => setGainPct(v as number)}
              min={10}
              max={80}
              step={5}
              aria-label="Assumed efficiency gain percent"
            />
          </div>
        </Stack>
